...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"""You are an impartial evaluator. Score each criterion from 0.0 to 1.0.
Return a single JSON object mapping criterion names to numeric scores. Do not
reward verbosity; judge only the stated rubric.
...
"""
        ...
        try:
            ...
            scores = {
                criterion: min(1.0, max(0.0, float(candidate[criterion])))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if isinstance(first, (int, float)) and isinstance(second, (int, float)):
                paired_differences.append(float(first) - float(second))
        positional_bias = bool(paired_differences) and all(
            difference > 0.05 for difference in paired_differences
        )
        return {
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if min(scores, key=scores.get) == "faithfulness":
            return "Context is missing or irrelevant — improve retrieval"
        if min(scores, key=scores.get) == "relevance":
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review and retest the failing case"
            values = [
                str(failure_id),
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            candidates.append((categories["hallucination"], "Add claim-level grounding checks and reject statements unsupported by retrieved evidence"))
        if categories.get("irrelevant"):
            candidates.append((categories["irrelevant"], "Add intent-focused prompt examples and verify every response directly addresses the question"))
        if categories.get("incomplete"):
            candidates.append((categories["incomplete"], "Improve retrieval coverage and require answers to preserve all dates, amounts, conditions, and exceptions"))
        if categories.get("off_topic"):
            candidates.append((categories["off_topic"], "Add intent routing and an explicit out-of-scope response policy"))
        if categories.get("refusal"):
            candidates.append((categories["refusal"], "Calibrate safety rules with allowed student-services examples to reduce false refusals"))

        defaults = [
            (0, "Tune BM25 query expansion and chunk boundaries, then re-run Context Recall and Context Precision"),
            (0, "Add the failing traces to the regression suite and block releases when core metrics drop by more than 0.05"),
            (0, "Review the three lowest-scoring answers against gold evidence with a calibrated human rubric"),
        ]
        ...
        return suggestions[: max(3, min(len(suggestions), len(failures)))]
